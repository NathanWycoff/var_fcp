...
class SblNet(object):
    def __init__(self, X, y, XX = None, penalty = 'laplace', plotname = 'traj.pdf', add_intercept = True, scale = True):
        ...
        if scale:
            ...
            int_modif = eta[1:] @ (mu_X/sig_X)
            eta = eta.at[0].set(sig_y*eta[0] + mu_y - sig_y * int_modif)
            eta = eta.at[1:].set(sig_y / sig_X * eta[1:])
...
if __name__=='__main__':
    ...
    intercept = 1000.
    y = X[:,0] + np.random.normal(scale=sigma2_true,size=N) + intercept
    yy = XX[:,0] + np.random.normal(scale=sigma2_true,size=N) + intercept
    ...
    #print(np.sum(np.square(beta_hat[1:] - beta_true)))
